 ...
 * 	发布给所有的订阅者，无回包。
 *********/
func (a *AmqClientPublisher) PubUnreliable2AllMessage(data []byte, cmd anet.PackHeadCmd) (err error) {
 ...
 *		或者超时。
 *********/
func (a *AmqClientPublisher) PubReliable2RandomOneMessage(data []byte, cmd anet.PackHeadCmd) (pack *anet.PackHead, isTimeout bool) {
 ...
 *		或者超时。
 *********/
func (a *AmqClientPublisher) PubReliable2SpecOneMessage(data []byte, cmd anet.PackHeadCmd) (pack *anet.PackHead, isTimeout bool) {
 ...
 *		或者超时。
 *********/
func (a *AmqClientPublisher) PubUnreliable2RandomOneMessage(data []byte, cmd anet.PackHeadCmd) (err error) {
	msg := &anet.PackHead{ReserveLow: AmqCmdDefReliable2RandomOne, Cmd: cmd,
		SequenceID: 0,
		Length:     uint32(len(data)), Body: data}
	_, err = a.conn.WriteMessage(msg)
