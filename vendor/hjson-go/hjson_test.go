...
func getTestContent(name string) []byte {
	p := fmt.Sprintf("./pokermainassets/%s_test.hjson", name)
	if _, err := os.Stat(p); os.IsNotExist(err) {
		p = fmt.Sprintf("./pokermainassets/%s_test.json", name)
	}
	return getContent(p)
}

func getResultContent(name string) ([]byte, []byte) {
	p1 := fmt.Sprintf("./pokermainassets/sorted/%s_result.json", name)
	p2 := fmt.Sprintf("./pokermainassets/sorted/%s_result.hjson", name)
...
func TestFromFile(t *testing.T) {
	...
	files := strings.Split(string(getContent("sample.hjson")), "\n")
	fmt.Println(files)
	for _, file := range files {
		if !strings.HasPrefix(file, "stringify/quotes") && !strings.HasPrefix(file, "extra/") {
			run(t, file)
...
func ParseHjson(filePath string, data interface{})  {
	files := strings.Split(string(getContent(filePath)), "\n")
	fmt.Println(files)
	for _, file := range files {
		if !strings.HasPrefix(file, "stringify/quotes") && !strings.HasPrefix(file, "extra/") {
			run(t, file)
